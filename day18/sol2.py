...
def corrupting_byte(step: int) -> [Coordinates]:
    number_of_steps = 1
...
while can_go:
    bytes += 1
    all_paths = find_all_paths(START_COORD, ALL_BYTES[:bytes])
    dj, bt = dijkstra(all_paths, START_COORD)

    if END_COORD not in dj.keys():
        can_go = False
        print(f'-------------{bytes} which is {ALL_BYTES[bytes - 1].second, ALL_BYTES[bytes -1].first}')
    else:
        print(f'bytes is {bytes}: {ALL_BYTES[bytes].second, ALL_BYTES[bytes].first} and path is {dj[END_COORD]}')
