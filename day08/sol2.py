...
class Antena:
    ...
    def get_antinodes(self,
        ...
        if self.first_coordinate <= other_antena.first_coordinate and self.second_coordinate <= other_antena.second_coordinate:
            while True:
                this_fc_dist = i_itr * fc_dist
                this_sc_dist = i_itr * sc_dist

                p1 = (self.first_coordinate - this_fc_dist, self.second_coordinate - this_sc_dist)
                p2 = (other_antena.first_coordinate + this_fc_dist, other_antena.second_coordinate + this_sc_dist)

                if not check_if_in_place(ts_place, p1) and not check_if_in_place(ts_place, p2):
                    return output
...
for i in range(len(antenas)):
    ...
    for other_antena_itm in [x for j, x in enumerate(antenas) if j != i]:
        if this_antena.letter == other_antena_itm.letter:
            ...
            new_antinodes = this_antena.get_antinodes(other_antena_itm, place)
            # print(new_antinodes)
